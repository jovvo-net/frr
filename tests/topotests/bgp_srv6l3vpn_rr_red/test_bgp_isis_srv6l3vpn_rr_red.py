...
def check_kernel_seg6_encap_red(rname, match):
    count = 0
    tgen = get_topogen()
    while count < 60:
        res = tgen.gears[rname].run("ip nexthop | grep \"encap.red\"")
        logger.info("count-{} result {}".format(count, res))
        if res:
            if match in res:
                return True
...
def test_kernel_state():
    ...
    logger.info("Check r1 kernel seg6 encap.red")
    if not check_kernel_seg6_encap_red("r1", "fcff:0:4:1::"):
        assert False, "r1 doesn't contains seg6 encap.red [fcff:0:4:1::]"
    if not check_kernel_seg6_encap_red("r1", "fcff:0:4:2::"):
        assert False, "r1 doesn't contains seg6 encap.red [fcff:0:4:2::]"
    if not check_kernel_seg6_encap_red("r1", "fcff:0:4:3::"):
        assert False, "r1 doesn't contains seg6 encap.red [fcff:0:4:3::]"
    if not check_kernel_seg6_encap_red("r1", "fcff:0:4:4::"):
        assert False, "r1 doesn't contains seg6 encap.red [fcff:0:4:4::]"

    logger.info("Check r4 kernel seg6 encap.red")
    if not check_kernel_seg6_encap_red("r4", "fcff:0:1:1::"):
        assert False, "r4 doesn't contains seg6 encap.red [fcff:0:1:1::]"
    if not check_kernel_seg6_encap_red("r4", "fcff:0:1:2::"):
        assert False, "r4 doesn't contains seg6 encap.red [fcff:0:1:2::]"
    if not check_kernel_seg6_encap_red("r4", "fcff:0:1:3::"):
        assert False, "r4 doesn't contains seg6 encap.red [fcff:0:1:3::]"
    if not check_kernel_seg6_encap_red("r4", "fcff:0:1:4::"):
        assert False, "r4 doesn't contains seg6 encap.red [fcff:0:1:4::]"
